...
The monitor URL is scoped to the current OAuth session and PR, carries no GitHub credential, and expires 12 hours after `open_pr_monitor` returns it. Only `GET /monitor/...` is accepted. The feed replays events after the URL's `cursor` (or the `Last-Event-ID` header), then remains open for live events and heartbeat comments. Each event includes size-bounded `details` for actionable state: check waves, named failures and cancellations with URLs, head/base/rebase changes, and the bodies and IDs of changed comments, reviews, and review comments. Routine check churn has empty details. A cursor older than the bounded event history receives a reconciliation event from the current snapshot. Scheduled polling removes expired capability records; `unwatch_pr` revokes active capabilities, and merged or closed feeds emit their terminal event and close naturally.
