...
describe("GitHub API adapter", () => {
  it("normalizes pull state, comments, reviews, checks, reactions, and threads", async () => {
    ...
    globalThis.fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith("/commits/abc/check-runs")) return Response.json({ check_runs: [{ id: 4, name: "CI", status: "completed", conclusion: "failure", completed_at: "now", started_at: "then", html_url: "https://github.com/check" }] });
